...
shinyServer(function(input, output){      
    ...
    react1<- reactive({ 
      ...
      if(input$MTCARS){tabl1<-BestFit(mtcars,input$sliderNumVar,input$CRITERA)
      }
      if(input$AIRQUALITY){tabl1<-BestFit(airquality,input$sliderNumVar,input$CRITERA)
      }
      if(input$DIAMONDS){tabl1<-BestFit(diamonds,input$sliderNumVar,input$CRITERA)
      }
      if(input$STATE.X77){tabl1<-BestFit(state<-as.data.frame(state.x77),input$sliderNumVar,input$CRITERA)
      }
      tabl1
    })          
